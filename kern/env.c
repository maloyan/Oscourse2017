...
bind_functions(struct Env *e, struct Elf *elf)
{
	...
	//указатель на начало массива со структурами
	//elf->e_eshoff holds section header table’s file offset in bytes
	struct Secthdr *sh_start = (struct Secthdr *)((uint8_t *)elf + elf->e_shoff);
	//указатель на конец массива структур
	//elf->e_shnum holds the number of entries in the section header table
	...
	char *section_name = (char *)elf + sh_start[elf->e_shstrndx].sh_offset;
	char *strtab = NULL;
	struct Elf32_Sym *sym_start = NULL;
	struct Elf32_Sym *sym_end = NULL;
	struct Elf32_Sym *sym;
	uintptr_t addr;

	for (sh = sh_start; sh < sh_end; sh++) {
		if (!strcmp(&section_name[sh->sh_name], ".strtab")) {
			strtab = (char *) elf + sh->sh_offset;
		} else if (!strcmp(&section_name[sh->sh_name], ".symtab")) {
